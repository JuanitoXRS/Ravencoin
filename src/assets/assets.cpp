...
{
    // Issuing a new Asset must contain at least 3 CTxOut ( Raven Burn Tx, Any Number of other Outputs ..., Issue Asset Tx, Owner Asset Tx)
    if (vout.size() < 3)
        return false;
...
bool CAssetsCache::RemoveReissueAsset(const CReissueAsset& reissue, const std::string address, const COutPoint& out, const std::vector<std::pair<std::string, std::string> >& vUndoIPFS)
{
    ...
    // TODO get the old ipfs hash from the undoblock
